...
class ProjectsApiTests(APITestCase):
    ...
    def test_bootstrap_demo_command_creates_reusable_sample_data(self):
        ...
        project = ConstructionProject.objects.get(code='OBR-001')
        material = ProjectMaterial.objects.get(project=project, delivery_note='ALB-0001')

        self.assertTrue(engineer.groups.filter(name=ENGINEERS_GROUP).exists())
        self.assertTrue(worker.groups.filter(name=WORKERS_GROUP).exists())
        self.assertTrue(Token.objects.filter(user=engineer).exists())
        self.assertTrue(Token.objects.filter(user=worker).exists())
        self.assertEqual(material.id, ProjectMaterial.objects.get(id=material.id).id)
